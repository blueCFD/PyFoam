...
class DisplayBlockMeshDialog(QtGui.QMainWindow):
    ...
    def showTmpBlock(self):
        ...
        b=self.tmpBlock
        append.AddInputData(self.makeFace([b[0],b[1],b[2],b[3]]))
        append.AddInputData(self.makeFace([b[4],b[5],b[6],b[7]]))
        append.AddInputData(self.makeFace([b[0],b[1],b[5],b[4]]))
        append.AddInputData(self.makeFace([b[3],b[2],b[6],b[7]]))
        append.AddInputData(self.makeFace([b[0],b[3],b[7],b[4]]))
        append.AddInputData(self.makeFace([b[1],b[2],b[6],b[5]]))
        mapper=vtk.vtkPolyDataMapper()
    ...
    def colorBlock(self,value):
        ...
        if newBlock>=0:
            ...
            append.AddInputConnection(d1.GetOutputPort())
            self.ren.AddActor(t1)
            d2,t2=self.makeDirection(b[0],b[3],"x2")
            append.AddInputConnection(d2.GetOutputPort())
            self.ren.AddActor(t2)
            d3,t3=self.makeDirection(b[0],b[4],"x3")
            append.AddInputConnection(d3.GetOutputPort())
    ...
    def colorPatch(self,value):
        ...
        if newPatch>=0:
            ...
            append=vtk.vtkAppendPolyData()
            for s in subs:
                append.AddInputData(self.makeFace(s))
            mapper=vtk.vtkPolyDataMapper()
            mapper.SetInputConnection(append.GetOutputPort())
